...
public class UserController {
    ...
    public ResponseEntity<BirthdayResponse> register(@Valid @RequestBody UserRequest userRequest) {
        return new ResponseEntity<>(userService.saveUser(userRequest), HttpStatus.NO_CONTENT);
    }

    @PutMapping(path = "/update/{userId}")
    @ApiOperation(value = "Updates an existing User")
    public ResponseEntity<BirthdayResponse> update(@PathVariable("userId")Long id, UserRequest newUserRequest) {
